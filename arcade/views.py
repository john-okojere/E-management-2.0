...
def sales_history(request):
    ...
    if request.user.role == "Cashier":
        ...
        sales = Sale.objects.filter(paid=True, cashier=request.user, day=day).order_by('-date')

    elif request.user.role == "Manager" or request.user.level >= 3:
        sales = Sale.objects.filter(paid=True).order_by('-date')
    
    else:
        sales = Sale.objects.none()  # Define sales as an empty queryset for other roles
