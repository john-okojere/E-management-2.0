...
def manage_approvals(request):
    ...
    return render(request, "fashion/approvals/manage_approvals.html", context)
...
def end_of_day_report(request):
    ...
    return render(request, 'fashion/end-of-day.html', context)
...
def dashboard(request):
    ...
    print(context)
    return render(request, "fashion/dashboard.html", context)
...
def approve_day(request, day_id):
    ...
    if request.method == "POST":
        ...
        return redirect("fashion_manage_approvals")
    # Calculate metrics
    sales = Sale.objects.filter(day=day)
    total_sales = sales.aggregate(total=Sum('total'))['total'] or Decimal('0.00')
    total_completed_sales = sales.filter(completed=True).count()
    ...
    return render(request, "fashion/approvals/approve_day.html", context)
...
def approve_discount(request, discount_id):
    ...
    if request.method == "POST":
        ...
        return redirect("fashion_manage_approvals")
    return render(request, "fashion/approvals/approve_discount.html", {"discount": discount, 'discount_percentage': round(discount_percentage, 2)})
...
def upload_inventory(request):
    if request.method == 'POST' and request.FILES.get('file'):
        try:
            ...
            return redirect('fashion_inventory_list')  # Redirect to the inventory page

        except Exception as e:
            return JsonResponse({'status': 'error', 'message': str(e)})

    return render(request, 'fashion/inventory/upload_inventory.html')
...
def update_sale_item(request, sale_item_id):
    ...
    try:
        ...
        sale = sale_item.sale
        sale.total = sum(item.total for item in sale.fashion_items.all())
        sale.save()
