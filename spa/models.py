...
class Day(models.Model):
    staff = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='spa_days')
...
class Inventory(models.Model):
    staff = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='spa_inventories')
    name = models.CharField(max_length=100)
...
class Sale(models.Model):
    cashier = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="spa_sale")  # Who handled the sale
...
class SaleDiscount(models.Model):
    cashier = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='spa_discounts')
    ...
    approved_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, blank=True, related_name='spa_approver')

class SaleItemDiscount(models.Model):
    cashier = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='spa_saleitem_discounts')
    sale = models.ForeignKey(SaleItem, on_delete=models.CASCADE, related_name='spa_discounts')
    proposed_discount = models.DecimalField(max_digits=10, decimal_places=2)
    approved = models.BooleanField(default=False)
    approved_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, blank=True, related_name='spa_saleitem_approver')
...
class Payment(models.Model):
    ...
    cashier = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='spa_cashier_payments')
